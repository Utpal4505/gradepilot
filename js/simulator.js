...
function updateUI(container) {
  ...
  const isPositive = deltaValue > 0.005;
  ...
  const subjectRows = subjects.map(sub => {
    ...
    if (isSimulated) {
      ...
      let impClass = 'text-slate-500 bg-slate-50 dark:bg-slate-700/50';
      if (impPositive) impClass = 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30';
      else if (impNegative) impClass = 'text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/30';
    ...
    const rowBg = isSimulated
      ? 'bg-indigo-50/40 dark:bg-indigo-900/10 border-indigo-200 dark:border-indigo-800/50'
      : 'bg-slate-50 dark:bg-slate-700/30 border-slate-200 dark:border-slate-700';

    const selectBorder = isSimulated
      ? 'border-indigo-300 dark:border-indigo-500 text-indigo-700 dark:text-indigo-400 focus:ring-indigo-500'
      : 'border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 focus:ring-slate-500';

    const options = SELECTABLE_GRADES.map(g => {
      const sel = g.grade === simulated ? 'selected' : '';
      return '<option value="' + g.grade + '" ' + sel + '>' + g.grade + '</option>';
    }).join('');

    return '<div class="flex flex-col sm:flex-row sm:items-center gap-3 p-3 rounded-xl border ' + rowBg + ' transition-all duration-200">' +
      '<div class="flex-1 min-w-0">' +
        '<div class="font-medium text-slate-800 dark:text-slate-200 truncate">' + sub.name + '</div>' +
      '</div>' +
      '<div class="flex items-center gap-2 sm:gap-3">' +
        '<div class="flex items-center justify-center w-10">' +
          '<span class="px-2 py-1 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 text-sm font-semibold rounded-md shadow-sm">' + sub.expectedGrade + '</span>' +
        '</div>' +
        '<svg class="w-4 h-4 text-slate-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>' +
        '<select data-id="' + sub.id + '" class="sim-select bg-white dark:bg-slate-800 border ' + selectBorder + ' text-sm rounded-lg font-semibold focus:outline-none focus:ring-2 shadow-sm py-1.5 pl-3 pr-8 cursor-pointer transition-colors duration-200">' + options + '</select>' +
        '<div class="w-14 flex justify-end shrink-0">' + impactHTML + '</div>' +
      '</div>' +
    '</div>';
  }).join('');
  ...
  if (hasAnySimulation) {
    beforeAfterHTML =
      '<span class="text-xl font-bold text-slate-400 dark:text-slate-500 line-through decoration-slate-300 dark:decoration-slate-600">' + currentSGPA.toFixed(2) + '</span>' +
      '<span class="text-lg text-slate-400 dark:text-slate-500">\u2192</span>';
  }

  let deltaHTML = '';
  if (hasAnySimulation) {
    deltaHTML = '<span class="text-sm font-bold ' + deltaClass + ' ml-1">(' + deltaSign + delta + ')</span>';
  }

  const resetDisabled = !hasAnySimulation;
  const resetClasses = resetDisabled
  ...
  container.innerHTML =
    '<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 flex flex-col h-full transition-colors duration-200">' +
      '<div class="mb-6">' +
        ...
        '</h2>' +
        '<p class="text-slate-500 dark:text-slate-400 text-sm">Play with grades and see how your SGPA changes instantly</p>' +
      '</div>' +
      '<div class="flex-1 overflow-y-auto pr-2 space-y-3 mb-6">' + subjectRows + '</div>' +
      '<div class="pt-5 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-auto">' +
        '<div class="flex flex-col">' +
          ...
          '</div>' +
        '</div>' +
        '<button id="sim-reset-btn" class="' + resetClasses + ' px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-semibold rounded-lg transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-slate-400 flex items-center justify-center gap-2"' + (resetDisabled ? ' disabled' : '') + '>' +
