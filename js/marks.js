...
import { getDifficulty, ASSESSMENT_SCHEMES, round } from './utils.js';
      ...
      <div class="card p-5 sm:p-6 transition-all duration-200 border border-slate-200 dark:border-slate-700">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <div class="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-2">
              <span>Scheme: <strong>${scheme.name}</strong></span>
              <span>•</span>
              <span>Current Attempted: <strong>${evalData.currentScored} / ${evalData.currentMax}</strong></span>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 my-5">
          ...
          <div class="p-3.5 bg-indigo-50/50 dark:bg-indigo-950/30 rounded-xl border border-indigo-200 dark:border-indigo-800/60">
            <div class="flex items-center justify-between mb-1.5">
              ...
              <span class="text-[11px] text-indigo-600 dark:text-indigo-400">Max: ${evalData.endtermMax}</span>
            </div>
            <div class="flex items-center gap-2">
              <input type="number" step="0.5" min="0" max="${evalData.endtermMax}" value="${endTarget}"
                data-subid="${sub.id}" data-comp="endterm"
                class="mark-input w-full px-3 py-1.5 text-sm font-semibold rounded-lg border border-indigo-300 dark:border-indigo-600 bg-white dark:bg-slate-900 text-indigo-950 dark:text-indigo-100 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
        ...
        <div class="bg-slate-50 dark:bg-slate-800/40 rounded-xl p-3.5 border border-slate-100 dark:border-slate-800">
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-[11px] text-slate-400">Based on CA + Midterm total (${evalData.currentScored}/${evalData.currentMax})</span>
          </div>
